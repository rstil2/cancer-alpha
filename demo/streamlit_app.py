...
st.set_page_config(
    page_title="Oncura — Cancer Genomics Demo",
    page_icon="🧬",
    layout="wide",
...
def main():
    ...
    # App header
    st.title("Oncura — Multi-modal Cancer Classification Demo")
    st.markdown("""
    **Workflow:** input genomic features → classify cancer type → inspect confidence and SHAP explanations.

    Demo models (Logistic Regression & Random Forest) were trained on 158 real TCGA samples with 110
  multi-modal features. Use **Sample Data** for a guided walkthrough.
    """)

    with st.expander("Research pipeline (separate from this demo)"):
        st.markdown("""
        - **Study 2:** LightGBM **98.4%** held-out balanced accuracy (n=1,248, 4,063 features)
        - Reproduce: `python src/pipeline/step4_train_evaluate.py`
        - Details: [CANONICAL.md](https://github.com/rstil2/cancer-alpha/blob/main/docs/CANONICAL.md)
        """)
