...
"""
🔥 PRODUCTION-GRADE TCGA DATA DOWNLOADER 🔥
=============================================

Conservative, rate-limited downloader designed for long-term stability
and compliance with GDC API best practices.
...
class RateLimitedSession:
    ...
    def __init__(self, requests_per_second: float = 2.0, max_retries: int = 3):
        ...
        self.session.headers.update({
            'User-Agent': 'Cancer-Alpha-Production-Downloader/1.0',
...
class ProductionTCGADownloader:
    """Production-grade TCGA downloader with conservative rate limiting"""
    ...
    def __init__(self, base_dir: str = "data/production_tcga", max_workers: int = 3):
        ...
        # TCGA projects (conservative list - most reliable)
        self.tcga_projects = [
            "TCGA-BRCA",  # Breast
        ...
        logger.info("🔥 Production TCGA Downloader initialized")
        logger.info(f"📂 Base directory: {self.base_dir}")
        logger.info(f"🧵 Max workers: {self.max_workers}")
        logger.info(f"⚡ Rate limit: 1.5 requests/second")
        logger.info(f"🎯 Target projects: {len(self.tcga_projects)}")
    ...
    def get_project_files(self, project: str, data_type: str, max_files: int = 1000) -> List[Dict]:
    ...
    def download_project_data(self, project: str, data_type_key: str, max_files: int = 500):
    ...
    def download_all_data(self):
        ...
        logger.info("🚀 Starting production TCGA download")
        ...
        if not self.shutdown_requested:
            logger.info("🎉 Production download completed successfully!")
        else:
...
def main():
    """Main entry point"""
    print("🔥 PRODUCTION TCGA DOWNLOADER 🔥")
    print("================================")
    print("Conservative, rate-limited downloader for production use")
    print("100% REAL DATA - ZERO SYNTHETIC CONTAMINATION")
    print()
    
    downloader = ProductionTCGADownloader()
